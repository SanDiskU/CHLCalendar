...
@interface HCGResultVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
- (void)backToSuper
{
    ...
    if ([str isEqualToString:@"camera"]) {
        for (UIViewController *temp in self.navigationController.viewControllers) {
            if ([temp isKindOfClass:[HCGPaperVC class]]) {
...
- (void)saveImageBtnClick
{
    ...
    //相机保存
    if ([str isEqualToString:@"camera"]) {
        [[NSNotificationCenter defaultCenter]postNotificationName:@"HCGcamera" object:self userInfo:@{@"image":self.resultImage,@"time":_nowStr,@"value":_textValueStr}];
        for (UIViewController *temp in self.navigationController.viewControllers) {
            if ([temp isKindOfClass:[HCGPaperVC class]]) {
                [self.navigationController popToViewController:temp animated:YES];
            }
        ...
        //相册保存
        if ([_delegate respondsToSelector:@selector(getHCGResultWithImage:andTime:andValue:)]) {
            [_delegate getHCGResultWithImage:self.resultImage andTime:_nowStr andValue:_textValueStr];
            [self.navigationController popViewControllerAnimated:YES];
        }
    }
}
...
- (void)styleDelare
{
    ...
    //强阳，阳性，阴性，无效
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 244, __kScreenWidth, 180)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *array1 = @[@"阳性",@"阴性",@"无效"];
    NSArray *array2 = @[@"恭喜您怀孕了",@"继续加油",@"有点小问题，再试试"];
    NSArray *array3 = @[image1,image5,image3];
    NSArray *array4 = @[image6,image4];
    ...
    if (indexPath.row == 1||indexPath.row == 2) {
        ...
        imageView.image = array4[indexPath.row-1];
        [cell addSubview:imageView];
    }
