...
@interface DeleteHCGPaperVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation DeleteHCGPaperVC
...
- (void)changeTextPaper
{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"changeHCGTextPaper" object:self userInfo:@{@"time":_nowStr,@"value":_textValueStr,@"textId":textid}];
    [self.navigationController popViewControllerAnimated:YES];
}
...
{
    NSString *textid = [NSString stringWithFormat:@"%ld",self.textId];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"deleteHCGTextPaper" object:self userInfo:@{@"textId":textid}];
    [self.navigationController popViewControllerAnimated:YES];
}
